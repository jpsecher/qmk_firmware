...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
  [_ALP_MAC] = LAYOUT(
    ...
    TAB_SFT,  KC_A,     S_ALT,    D_GUI,    F_CTL,    KC_G,      KC_H,     J_CTL,    K_GUI,    L_ALT,    KC_B,     KC_ENT,
              ...
              KC_MPLY,  ESC_AGR,  MO_NAV,   OSM_LSFT,                      OSL_SYM,  KC_SPC,   KC_BSPC,  KC_MNXT
  ),
  [_NAV_MAC] = LAYOUT(
  ...
  [_NUM_MAC] = LAYOUT(
    ...
    _______,  F5_AGR,   F6_ALT,   F7_GUI,   F8_CTL,   KC_MINS,   KC_0,     FOUR_CTL, FIVE_GUI, SIX_ALT,  KC_PLUS,  _______,
              ...
              KC_EJCT,  KC_UNDS,  LCK_ALPM, KC_SLSH,                       KC_COMM,  _______,  _______,  KC_PWR
  ),
  [_SYMBOLS] = LAYOUT(
              KC_SPC,   KC_LCBR,  KC_QUOT,  KC_RCBR,  KC_EQL,    KC_COLN,  KC_AMPR,  KC_ASTR,  KC_QUES,  QK_LLCK,
    KC_TILD,  KC_SCLN,  KC_LPRN,  KC_DQUO,  KC_RPRN,  KC_MINS,   KC_0,     KC_DLR,   KC_PERC,  KC_CIRC,  KC_PLUS,  _______,
              KC_BSLS,  KC_LBRC,  KC_GRV,   KC_RBRC,                       KC_EXLM,  KC_AT,    KC_HASH,  KC_PIPE,
              KC_VOLD,  KC_UNDS,  LCK_ALPM, KC_SLSH,                       KC_COMM,  _______,  _______,  KC_VOLU
  ),
  [_ALP_OVL] = LAYOUT(
              KC_Q,     KC_W,     KC_E,     KC_R,     KC_T,      KC_Y,     KC_U,     KC_I,     KC_O,     KC_P,
    _______,  KC_A,     KC_S,     KC_D,     KC_F,     KC_G,      KC_H,     KC_J,     KC_K,     KC_L,     KC_B,     _______,
              KC_Z,     KC_X,     KC_C,     KC_V,                          KC_N,     KC_M,     KC_COMM,  KC_DOT,
              _______,  _______,  LCK_ALPM, _______,                       _______,  _______,  _______,  _______
  )
};
...
const rgblight_segment_t PROGMEM symb_layer[] = RGBLIGHT_LAYER_SEGMENTS({0, 6, HSV_CYAN});
const rgblight_segment_t PROGMEM func_layer[] = RGBLIGHT_LAYER_SEGMENTS({0, 6, HSV_CORAL});
const rgblight_segment_t PROGMEM caps_layer[] = RGBLIGHT_LAYER_SEGMENTS({0, 6, HSV_GOLD});
